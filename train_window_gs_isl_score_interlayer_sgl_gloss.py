...
#  固定层间链路，只规划层间和星地链路
#  星地链路按地面站层与LEO/MEO/GEO连，而不是单个的地面站分别与
...
from models.edge_q import EdgeQNet
...
def choose_action_from_state(model: EdgeQNet, device: torch.device, amp_enabled: bool,
...
def main():
    ...
    EDGE_DIM = 6
    model = EdgeQNet(edge_dim=EDGE_DIM, hidden=cfg.HIDDEN_DIM).to(device)
    target = EdgeQNet(edge_dim=EDGE_DIM, hidden=cfg.HIDDEN_DIM).to(device)
...
# ===== GL helpers: generalized loss & beta selection =====
def _gl_loss(delta: torch.Tensor, alpha: float, beta: torch.Tensor) -> torch.Tensor:
    """
    L_GL(δ) = β^α * (1 - exp(-( |δ|/β )^α))
    支持标量/逐元素 beta（广播）；返回逐元素 loss（不做 reduce）
    """
    delta_abs = delta.abs()
    beta_safe = torch.clamp(beta, min=getattr(cfg, "GL_BETA_EPS", 1e-6))
    z = (delta_abs / beta_safe).pow(alpha)
    return beta_safe.pow(alpha) * (1.0 - torch.exp(-z))


# 保存一个简单的 EMA 状态（进程内持久），用于 beta 的平滑可选
_beta_ema = {"init": False, "val": None}

def choose_beta(delta_vec: torch.Tensor) -> torch.Tensor:
    """
    从本批 TD 误差中选 β：
      - fixed  : 直接用 cfg.GL_BETA_FIXED
      - median : β = cfg.GL_BETA_SCALE * median(|δ|)
      - ema    : 在 median 基础上做 EMA 平滑
    返回: 标量 tensor（按批共享 β，更稳）
    """
    mode = str(getattr(cfg, "GL_BETA_MODE", "median")).lower()
    if mode == "fixed":
        return torch.as_tensor(getattr(cfg, "GL_BETA_FIXED", 1.0),
                               dtype=delta_vec.dtype, device=delta_vec.device)
    # median/ema
    med = delta_vec.detach().abs().median()
    beta_now = (getattr(cfg, "GL_BETA_SCALE", 2.0) * med).clamp_min(getattr(cfg, "GL_BETA_EPS", 1e-6))
    if mode == "ema":
        decay = float(getattr(cfg, "GL_BETA_EMA_DECAY", 0.95))
        if not _beta_ema["init"]:
            _beta_ema["val"] = beta_now
            _beta_ema["init"] = True
        else:
            _beta_ema["val"] = decay * _beta_ema["val"] + (1.0 - decay) * beta_now
        return _beta_ema["val"]
    else:
        return beta_now
...
def train_step(model, target, optim, scaler, device, amp_enabled, batch, replay=None, db=None):
    ...
    for seq in batch:
        ...
        # 2) 目标 Q（在 s_{t+n} 上，用 target Q + b-匹配 选动作）
        ...
        if feats_next.size == 0:
            ...
            with torch.no_grad():
                if amp_enabled:
                    ...
                    # with get_autocast(device=device, dtype=(
                    # torch.bfloat16 if getattr(torch.cuda, 'is_bf16_supported', lambda: False)() else torch.float16)):
                        q_next = target(x_next)
                else:
                    q_next = target(x_next)
            scores_next = {pairs_next[i]: float(q_next[i].item()) for i in range(len(pairs_next))}
            max_deg_next = make_max_deg(s_tpn.layer_map)
            # A_star = bmatch_greedy(scores_next, max_deg_next)
            ...
            # A_star = bmatch_with_dual_budgets(
            #     scores=scores_next,
            #     nodes=s_tpn.nodes,
            #     layer_map=s_tpn.layer_map,
            #     deg_isl_used=s_tpn.deg_isl_used,  # 由骨干+上一时刻仍可用边占用
            #     require_gs_per_layer=True,
            #     max_repair_steps=256
            # )
            ...
            # Q_boot = sum(scores_next[e] for e in A_star) if A_star else 0.0
            Q_boot = sum(scores_next[e] for e in A_star_interlayer) if A_star_interlayer else 0.0

            # Q_boot = sum(scores_next[e] for e in A_star)
        ...
        # 4) 单样本 TD 误差（GL 最后统一计算）
        q_est = q_est.float()
        y_t = y_t.float()
        delta_i = (y_t - q_est)  # TD 误差；GL 只用 |δ|
        valid_losses.append(delta_i)
    ...
    # 现在 valid_losses 里是若干 δ，把它们拼成一个 batch
    delta_batch = torch.stack(valid_losses)  # [B]

    use_gl = bool(getattr(cfg, "GL_USE", True))
    if use_gl:
        alpha = float(getattr(cfg, "GL_ALPHA", 1.6))
        beta = choose_beta(delta_batch)  # 标量 β（按批共享，更稳）
        loss_vec = _gl_loss(delta_batch, alpha=alpha, beta=beta)
        loss = loss_vec.mean()
    else:
        # 回退 Huber：与原实现等价（Huber(δ, 0)）
        loss = torch.nn.functional.huber_loss(
            delta_batch, torch.zeros_like(delta_batch), delta=1.0, reduction="mean"
        )
