...
def choose_action_from_state(model: EdgeDQNet, device: torch.device, amp_enabled: bool,
...
def main():
    ...
    EDGE_DIM = 6
    model = EdgeDQNet(edge_dim=EDGE_DIM, hidden=cfg.HIDDEN_DIM).to(device)
    target = EdgeDQNet(edge_dim=EDGE_DIM, hidden=cfg.HIDDEN_DIM).to(device)
