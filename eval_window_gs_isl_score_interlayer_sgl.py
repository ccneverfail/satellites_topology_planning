# eval_window_gs_isl_score_interlayer_sgl.py
# dueling DQN, double dueling dqn 都可以推理
...
# ---------------------------------------------------------------------------
# 服务器用的pytorch2.6有安全策略啥的，不允许datetime.timedelta反序列化，因此要加以下两行
# 如果不报错，可以将以下两行注释
import datetime
from torch.serialization import add_safe_globals
# ---------------------------------------------------------------------------
add_safe_globals([datetime.timedelta])
...
# ------------ 评估动作（含地面站/连通性修复） ------------
@torch.no_grad()
def _choose_action_from_state_eval(model: EdgeQNet, device: torch.device, amp_enabled: bool,
...
def eval_model(dir: str,
    ...
    model_net = EdgeQNet(edge_dim=edge_dim, hidden=hidden).to(device)
...
if __name__ == "__main__":
    ...
    out = eval_model(
        ...
        # dir="runs/20251019_210416",#Lookahead-D3QN
        # dir="runs/20251014_092716",#Dueling DQN
        # dir="runs/20251008_213602",#LMSR Dueling DQN
        # dir="runs/20251010_163752",#MSR Dueling DQN
        # dir="runs/20251127_171345",#133, GL+ Dueling DQN
        # dir="runs/20251127_221402",# 134, GL+ MSR-Dueling DQN
        dir="runs/20251128_111254",# 135, GL+ LMSR-Dueling DQN

        # dir="runs/20251011_204429",#MSR D3QN
        # dir="runs/20251124_215736",#126,ALPHA=1.2-CL+D3QN
        # dir="runs/20251012_105235",#89,LMSR D3QN111
        # dir="runs/20251022_014459",#108,LMSR D3QN222
        # dir="runs/20251022_112005",#108,LMSR D3QN333
        # dir="runs/20251028_120241",#111,correntropy loss, D3QN
        # dir="runs/20251104_184837",#112,correntropy loss, alpha=1.6,LMSR D3QN
        # dir="runs/20251115_181835",#118,correntropy loss，alpha=1.2, LMSR D3QN
        # dir="runs/20251116_234741",#120,correntropy loss，alpha=2.0, LMSR D3QN
        # dir="runs/20251117_104410",#121,correntropy loss，alpha=1.4, LMSR D3QN
        # dir="runs/20251117_191844",#122,correntropy loss，alpha=1.8, LMSR D3QN
        # dir="runs/20251118_024703",#123,correntropy loss，alpha=1.5, LMSR D3QN
        # dir="runs/20251118_114227",#124,correntropy loss，alpha=1.0, LMSR D3QN
        # dir="runs/20251105_222651",#11,correntropy loss, FW=FALSE,D3QN
        # dir="runs/20251014_140505",#D3QN
        name="epoch_035_step4165.pt",
        t_begin=0,  # 不传则用 ckpt/config

        t_end=240,  # 不传则用 ckpt/config
        ...
        # # 星间度约束
        # MAX_DEG_ISL_LEO=5,  # 5   LEO层卫星星间天线数量（5~8，静态链路已经占用了4个）
        # MAX_DEG_ISL_MEO=6,  # 6   MEO层卫星星间天线数量（5~8，大于等于5，静态链路已经占用了4个）
        # MAX_DEG_ISL_GEO=8,  # 8   GEO层卫星星间天线数量（3~8，大于等于3，静态链路已经占用了2个）
        #
        # #  星地度约束，SGL 在 卫星端/地面端占用 GSL 预算
        # MAX_DEG_SGL_LEO=3,  # 3   LEO层卫星星地天线数量（1~3）
        # MAX_DEG_SGL_MEO=3,  # 3  LEO层卫星星地天线数量（1~3）
        # MAX_DEG_SGL_GEO=3,  # 3  LEO层卫星星地天线数量（1~3）
        # MAX_DEG_SGL_GS=3,  # 3  # 地面站星地天线数量（1~6）  （每个地面站最多并发 SGL 条数）
        #
        # # 整个地面站在星地链路(SGL)上的“分层保底”配额，修改于20250930
        # REQ_SGL_LEO=1,  # 1  地面站层对LEO层卫星保底天线数量,大于等于1（1~3）
        # REQ_SGL_MEO=1,  # 1  地面站层对MEO层卫星保底天线数量,大于等于1（1~3）
        # REQ_SGL_GEO=1  # 1  地面站层对GEO层卫星保底天线数量,大于等于1（1~3）
