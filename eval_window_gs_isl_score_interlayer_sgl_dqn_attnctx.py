...
# attnctx DQN , double attnctx DQN 都可以推理
...
# ------------ 评估动作（含地面站/连通性修复） ------------
@torch.no_grad()
def _choose_action_from_state_eval(model: EdgeQAttnCtx, device: torch.device, amp_enabled: bool,
...
def _save_dynamic_topology_json(E_t: Set[Tuple[int, int]],
    ...
    E_t_remove_core = remove_E_core_graph(E_core= state.E_core,
                                          E_t=E_t)
...
def eval_model(dir: str,
    ...
    model_net = EdgeQAttnCtx(edge_dim=edge_dim, hidden=hidden).to(device)
...
if __name__ == "__main__":
    ...
    out = eval_model(
        ...
        # dir="runs/20251019_141843",#Lookahead-D3QN

        # dir="runs/20251017_103145",#LMSR-ATTEN-DDQN #99

        # dir="runs/20251019_000122",#ATTEN-DQN #101
        # dir="runs/20251020_122619",#fw-L-ATTEN-DQN #104

        # dir="runs/20251021_104443",#fw-ATTEN-DQN #107
        # dir="runs/20251128_171503",#CL1.2+fw-ATTEN-DQN #136
        # dir="runs/20251129_083741",#LMSR-fw-ATTEN-DQN #138
        # dir="runs/20251128_222841",#CL1.2+LMSR-fw-ATTEN-DQN #137

        # dir="runs/20251027_184906",#Correntropy loss, LMSR-ATTEN-DDQN #110
        # dir="runs/20251105_113932",# fw-ATTEN-DDQN #113
        # dir="runs/20251019_141843",#fw,MSR-ATTEN-DDQN, #102
        # dir="runs/20251019_210416",#fw,LMSR-ATTEN-DDQN, #103
        # dir="runs/20251106_120259",#Correntropy loss, CL1.6+fw-ATTEN-DDQN #115
        dir="runs/20251129_143136",#Correntropy loss, CL1.2+fw-ATTEN-DDQN #139
        # dir="runs/20251116_155632",#Correntropy loss, CL1.2+LMSR-fw-ATTEN-DDQN #119
        ...
        # # 星间度约束
        # MAX_DEG_ISL_LEO=5,  # 5   LEO层卫星星间天线数量（大于等于5，静态链路已经占用了4个）
        # MAX_DEG_ISL_MEO=6,  # 6   MEO层卫星星间天线数量（大于等于5，静态链路已经占用了4个）
        # MAX_DEG_ISL_GEO=8,  # 8   GEO层卫星星间天线数量（大于等于3，静态链路已经占用了2个）
        #
        # #  星地度约束，SGL 在 卫星端/地面端占用 GSL 预算
        # MAX_DEG_SGL_LEO=3,  # 3   LEO层卫星星地天线数量
        # MAX_DEG_SGL_MEO=3,  # 3  LEO层卫星星地天线数量
        # MAX_DEG_SGL_GEO=3,  # 3  LEO层卫星星地天线数量
        # MAX_DEG_SGL_GS=3,  # 3  # 地面站星地天线数量  （每个地面站最多并发 SGL 条数）
        #
